...
for index_mask in range(1):
    ...
    #torch.save(gcnv,'dblp50.pt')
    #torch.save(gcnv,dataname+'.pt')
    mugcn.load(fv,adjfv,adjv,gcnv)
    mugcn.test()
    #np.save(dataname+'_fv',fv.cpu())
    #np.save(dataname+'_adjfv',adjfv.cpu())
    #np.save(dataname+'_adjv',adjv.cpu())
...
# unit test for utils
# debug purpose only
#feat = genfeat(homo.x,homo.y,train_mask)
# genhetero(edge_indexv,edge_indexl,Yv)
# adj = from_edge_index_to_adj(homo.edge_index).shape)
# adj_test = torch.zeros(7+num_label,7+num_label)
# updatehetero(adj_test,adj)
# edge_index,edge_weight = normalize(homo.edge_index,None)
